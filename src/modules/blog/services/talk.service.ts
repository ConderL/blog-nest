...
export class TalkService {
  ...
  async remove(ids: number[]): Promise<void> {
    ...
    this.logger.log(`删除说说：ID=${ids.join(',')}`);
    try {
      const talks = await this.talkRepository.findBy({ id: In(ids) });
      if (talks.length > 0) {
        await this.talkRepository.remove(talks);
        this.logger.log(`删除说说成功，ID：${ids.join(',')}`);
      } else {
        this.logger.log(`未找到要删除的说说，ID：${ids.join(',')}`);
      }
    } catch (error) {
